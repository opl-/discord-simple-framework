class Command {
	...
	handle(event, message, args) {
		if (this.channelCooldown) {
			if (this._lastUse[event.channel.id] + this.channelCooldown > Date.now()) return event.delete();

			this._lastUse[event.channel.id] = Date.now();
		}

		if (this.userCooldown) {
			if (this._lastUse[event.author.id] + this.userCooldown > Date.now()) return event.delete();
