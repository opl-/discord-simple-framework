class Command {
	...
	handle(event, message, args) {
		if (this.channelCooldown) {
			if (this._lastUse[event.channel.id] + this.channelCooldown > Date.now()) return event.deleteMessage();

			this._lastUse[event.channel.id] = Date.now();
		}
		...
		if (this.channelReplyCooldown && event.channel.type !== 'dm') {
			if (this._lastReply[event.channel.id] + this.channelReplyCooldown > Date.now()) {
				event._forceDM = true;
			} else {
				this._lastReply[event.channel.id] = Date.now();
			}
		}
