class Command {
	constructor(bot, opts = {}) {
		...
		this.channelReplyCooldown = typeof(opts.channelReplyCooldown) === 'number' ? (opts.channelReplyCooldown > 0 ? opts.channelReplyCooldown : false) : false;
	...
	handle(event, message, args) {
		if (this.channelCooldown) {
			if (this._lastUse[event.channel_id] + this.channelCooldown > Date.now()) return event.deleteMessage();

			this._lastUse[event.channel_id] = Date.now();
		}

		if (this.userCooldown) {
			if (this._lastUse[event.author.id] + this.userCooldown > Date.now()) return event.deleteMessage();

			this._lastUse[event.author.id] = Date.now();
		}

		if (this.channelReplyCooldown && this.bot.channels.get(event.channel_id) && this.bot.channels.get(event.channel_id).type !== 'dm') {
			if (this._lastReply[event.channel_id] + this.channelReplyCooldown > Date.now()) {
				event._forceDM = true;
			} else {
				this._lastReply[event.channel_id] = Date.now();
			}
		}
